...
```text
/mic pause       마이크 트랙을 mute하고 Local VAD 타이머를 멈춤
/mic resume      같은 Realtime 연결에서 마이크와 VAD 재개
...
3. 얼굴 감지 threshold, Local VAD threshold, wakeword 정규화는 실제 녹음 테스트 없이 임의로 바꾸지 않는다.
